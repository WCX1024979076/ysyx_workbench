...
class IDU extends Module {
  val io = IO(new Bundle {
    ...
    val IFUFlag = Input(UInt(1.W))
    val EXUClear = Input(UInt(1.W))
    ...
    val EbreakIn = Output(UInt(1.W))
    val PcVal = Output(UInt(64.W))
    val IDUFlag = Output(UInt(1.W))
  })
  ...
  val AluOpReg = RegInit(0.U(5.W)) 
  val AluSrc1OpReg = RegInit(0.U(5.W))
  val AluSrc2OpReg = RegInit(0.U(5.W))
  val PcSrcReg = RegInit(0.U(5.W))
  val RinCtlReg = RegInit(0.U(4.W))
  val MemMaskReg = RegInit(0.U(8.W))
  val RdestReg = RegInit(0.U(5.W))
  val R1Reg = RegInit(0.U(5.W))
  val R2Reg = RegInit(0.U(5.W))
  val ImmReg = RegInit(0.U(64.W))
  val IDUFlagReg = RegInit(0.U(1.W))
  val EbreakInReg = RegInit(0.U(1.W))
  ...
  {
    RdestReg := io.Inst(11,7);
    R1Reg := io.Inst(19,15);
    R2Reg := io.Inst(24,20);
    IDUFlagReg := (io.IFUFlag & (~io.EXUClear));
    EbreakInReg := MuxLookup(io.Inst,0.U,Array("h00100073".U -> 1.U));
    PcValReg := io.Pc;

    ImmReg := MuxLookup(opcode,0.U,Array(
      "b00101_11".U -> U(), //auipc
    ...
    RegWriteReg := contr_code(0);
    MemWriteReg := contr_code(1);
    AluSrc1OpReg := contr_code(2);
    AluSrc2OpReg := contr_code(3);
    AluOpReg := contr_code(4);
    PcSrcReg := contr_code(5);
    RinCtlReg := contr_code(6);
    MemMaskReg := contr_code(7);
  }
  
  io.RegWrite := RegWriteReg;
  io.MemWrite := MemWriteReg;
  io.AluOp := AluOpReg;
  io.AluSrc1Op := AluSrc1OpReg;
  io.AluSrc2Op := AluSrc2OpReg;
  io.PcSrc := PcSrcReg;
  io.RinCtl := RinCtlReg;
  io.MemMask := MemMaskReg;
  io.Rdest := RdestReg;
  io.R1 := R1Reg;
  io.R2 := R2Reg;
  io.Imm := ImmReg;
  io.EbreakIn := EbreakInReg;
  io.PcVal := PcValReg;
  io.IDUFlag := IDUFlagReg;
}