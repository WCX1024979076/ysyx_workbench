...
static word_t immI(uint32_t i) {Log("%x\n",i); return SEXT(BITS(i, 31, 20), 12); }
...
static word_t immJ(uint32_t i) {Log("%x\n",i); return (SEXT(BITS(i, 31, 31), 1) << 20) | (BITS(i, 30, 21) << 1) | (BITS(i,20,20) << 11) | (BITS(i,19,12) << 12); }
...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(dest) = s->pc + 4lu; s->dnpc = s->pc +(long long)SEXT(src1, 64); Log("%lx\n",src1));
